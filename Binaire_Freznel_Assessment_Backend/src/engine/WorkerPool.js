...
// Fixed-size pool of worker_threads that run chunk reductions.
//
// - runChunk() never blocks; it returns a promise that settles when a worker
//   is free and done.
// - A worker holds one chunk at a time and releases it immediately, so a task
//   never holds a worker across its whole file.
// - Workers never wait on each other.
// - A chunk over chunkTimeoutMs is killed and its worker replaced.
export class WorkerPool extends EventEmitter {
  ...
  #release(worker) {
    ...
    if (!meta) return; // already terminated
    meta.busy = false;
    meta.job = null;
    if (meta.timer) {
  ...
  #onMessage(worker, msg) {
    ...
    if (!meta || !meta.job || meta.job.jobId !== msg.jobId) {
      return; // stale message from a job we already timed out
    }
  ...
  async runChunk({ chunkIndex, lines, demoDelayMs }) {
    ...
    const meta = this.#workers.get(worker);
    if (!meta) {
      // Worker died between acquire and use; try again.
